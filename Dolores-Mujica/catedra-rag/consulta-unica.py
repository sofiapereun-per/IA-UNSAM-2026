#!/usr/bin/env python3
"""RAG mínimo - Asistente de Cátedra Universitario.

Qué hace RAG acá: en vez de mandarle al modelo todo el material de estudio,
buscamos primero los fragmentos parecidos a la pregunta del alumno y le mandamos SOLO esos.
"""
...
# Nombres de las secciones o unidades de tu texto
ENCABEZADOS = {
...
def partir_en_entradas(texto):
    """[1] Segmenta el archivo en fragmentos anteponiendo la unidad o sección."""
    fragmentos = []
    seccion = "General"
...
def vectorizar(textos):
    """[2] y [3] Convierte texto en vectores usando Ollama."""
    try:
...
def main():
    ...
    parser.add_argument("pregunta", help="Lo que le querés preguntar al asistente")
    ...
    if not args.datos.exists():
        sys.exit(f"No encuentro el archivo de la materia en {args.datos}")

    # [1] Leer y partir.
    fragmentos = partir_en_entradas(args.datos.read_text(encoding="utf-8"))
    print(f"[1] Leí el texto de la materia y lo partí en {len(fragmentos)} fragmentos.")
    ...
    # [4] Recuperar los top-k fragmentos.
    ...
    try:
        respuesta = ollama.chat(model=args.modelo, messages=[
            ...
            {"role": "user",
             "content": f"Fragmentos del texto de la materia:\n{recuperados}\n\nPregunta: {args.pregunta}"},
        ])
    except Exception as error:
